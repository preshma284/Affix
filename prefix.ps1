...
$excelFilePath = "C:\Users\preshma\OneDrive - Computer Enterprises Inc\Affix\Table-Prefix.xlsx"
...
foreach ($file in $files) {
    ...
    if ($firstLine -match $pattern) {
        ...
        if ($contentWithinQuotes -like "QB*") {
            ...
            $modifiedContent = "QBU " + $contentWithinQuotes
        }

        # Replace the original content within quotes with the modified content in the first line
        $modifiedFirstLine = $firstLine -replace $pattern, "`"$modifiedContent`""
        ...
        $excelData += [PSCustomObject]@{
            OriginalContent = $contentWithinQuotes
