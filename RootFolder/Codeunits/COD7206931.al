Codeunit 7206931 "QB Approval Prepayments"
{
    ...
    //[IntegrationEvent]
    PROCEDURE OnAfterSendApproval(VAR Rec: Record 7206928);
    BEGIN
    END;

    //[IntegrationEvent]
    PROCEDURE OnAfterReleaseDocument(VAR Rec: Record 7206928);
    BEGIN
    END;

    //[IntegrationEvent]
    PROCEDURE OnAfterReopenDocument(VAR Rec: Record 7206928);
    BEGIN
    END;

    //[IntegrationEvent]
    PROCEDURE OnAfterPerformManualReopenDoc(VAR Rec: Record 7206928);
    BEGIN
    END;
    ...
    PROCEDURE Release(VAR Rec: Record 7206928);
    VAR
    ...
    PROCEDURE Reopen(VAR Rec: Record 7206928);
    BEGIN
    ...
    PROCEDURE PerformManualRelease(VAR Rec: Record 7206928);
    VAR
    ...
    PROCEDURE PerformManualReopen(VAR Rec: Record 7206928);
    VAR
    ...
    PROCEDURE SetApprovalsControls(Rec: Record 7206928; VAR ApprovalsActive: Boolean; VAR OpenApprovalEntriesExistForCurrUser: Boolean; VAR CanRequestApproval: Boolean; VAR CanCancelApproval: Boolean; VAR CanRelease: Boolean; VAR CanReopen: Boolean);
    VAR
    ...
    PROCEDURE ViewApprovals(Rec: Record 7206928);
    VAR
        QBApprovalManagement: Codeunit 7207354;
        RecRef: RecordRef;
    BEGIN
    ...
    PROCEDURE SendApproval(Rec: Record 7206928);
    VAR
    ...
    PROCEDURE CancelApproval(Rec: Record 7206928);
    VAR
        WorkflowManagement: Codeunit 1501;
        WorkflowWebhookMgt: Codeunit 1543;
    BEGIN
    ...
    PROCEDURE WitHoldingApproval(Rec: Record 7206928);
    VAR
        QBApprovalManagement: Codeunit 7207354;
    ...
    BEGIN
        ...
        IF (RecRef.NUMBER = GetTableNumber) THEN BEGIN
            ...
            IF (TRUE) THEN BEGIN  //No hay varios tipos de documento en la misma tabla
                ...
                ApprovalEntryArgument.Type := 'Anticipo';
        ...
        IF (RecRef.NUMBER = GetTableNumber) THEN BEGIN
            ...
            IF (TRUE) THEN BEGIN  //No hay varios tipos de documento en la misma tabla
                IF (Rec."Approval Status" = Rec."Approval Status"::Released) THEN BEGIN
                    Field1Label := Rec.FIELDCAPTION("Entry No.");
                    Field1Value := FORMAT(Rec."Entry No.");
                    Field2Label := Rec.FIELDCAPTION(Description);
                    Field2Value := Rec.Description;
                END ELSE BEGIN
                    Field1Label := 'Importe';
                    Field1Value := FORMAT(Rec."Not Approved Amount");
                    Field2Label := FORMAT(Rec."Account Type");
                    Field2Value := '(' + FORMAT(Rec."Account No.") + ') ' + Rec."Account Description";
                END;
            END;
    ...
    PROCEDURE IsApprovalsWorkflowEnabled(VAR Rec: Record 7206928): Boolean;
    VAR
        WorkflowManagement: Codeunit 1501;
    BEGIN
    ...
    PROCEDURE CheckApprovalPossible(VAR Rec: Record 7206928): Boolean;
    VAR
    ...
    PROCEDURE PrePostApprovalCheck(VAR Rec: Record 7206928): Boolean;
    VAR
        PrePostCheckErr: TextConst ENU = '%1 %2 must be approved and released before you can perform this action.', ESP = 'El %1 %2 debe estar aprobado y lanzado antes de poder realizar esta acci�n.';
    BEGIN
        ...
        IF (Rec."Approval Status" = Rec."Approval Status"::Open) AND IsApprovalsWorkflowEnabled(Rec) THEN
            ERROR(PrePostCheckErr, 'Anticipo');
        EXIT(TRUE);
    END;
    ...
    BEGIN
        ...
        EXIT(QBApprovalManagement.GetApprovalsFixedText(pType, 'ANTICIPO', 'un anticipo'))
    END;
    ...
    BEGIN
        ...
        RecRef.SETTABLE(Rec);
        Amount := Rec."Not Approved Amount";
        AmountLCY := Rec."Not Approved Amount (LCY)";
    END;

    LOCAL PROCEDURE SetApprovalCircuit(VAR Rec: Record 7206928);
    VAR
        QBApprovalManagement: Codeunit 7207354;
        RecRef: RecordRef;
    BEGIN
    ...
    LOCAL PROCEDURE CheckAdditionalConditions(Rec: Record 7206928);
    BEGIN
    ...
    [EventSubscriber(ObjectType::Table, 7206928, OnAfterValidateEvent, "Job No.", true, true)]
    LOCAL PROCEDURE T7206928_OnAfterValidateEvent_JobNo(VAR Rec: Record 7206928; VAR xRec: Record 7206928; CurrFieldNo: Integer);
    BEGIN
        IF (TRUE) THEN  //No hay varios tipos de documento en la misma tabla
            SetApprovalCircuit(Rec);
    END;

    [EventSubscriber(ObjectType::Table, 7206928, OnAfterValidateEvent, "Budget Item", true, true)]
    LOCAL PROCEDURE T7206928_OnAfterValidateEvent_Budgetitem(VAR Rec: Record 7206928; VAR xRec: Record 7206928; CurrFieldNo: Integer);
    BEGIN
