Codeunit 7206921 "Approval Trans. Between Jobs"
{
    ...
    //[IntegrationEvent]
    PROCEDURE OnAfterSendApproval(VAR Rec: Record 7207286);
    BEGIN
    END;

    //[IntegrationEvent]
    PROCEDURE OnAfterReleaseDocument(VAR Rec: Record 7207286);
    BEGIN
    END;

    //[IntegrationEvent]
    PROCEDURE OnAfterReopenDocument(VAR Rec: Record 7207286);
    BEGIN
    END;

    //[IntegrationEvent]
    PROCEDURE OnAfterPerformManualReopenDoc(VAR Rec: Record 7207286);
    BEGIN
    END;
    ...
    PROCEDURE Release(VAR Rec: Record 7207286);
    BEGIN
    ...
    PROCEDURE Reopen(VAR Rec: Record 7207286);
    BEGIN
    ...
    PROCEDURE PerformManualRelease(VAR Rec: Record 7207286);
    VAR
    ...
    PROCEDURE PerformManualReopen(VAR Rec: Record 7207286);
    VAR
    ...
    PROCEDURE SetApprovalsControls(Rec: Record 7207286; VAR ApprovalsActive: Boolean; VAR OpenApprovalEntriesExistForCurrUser: Boolean; VAR CanRequestApproval: Boolean; VAR CanCancelApproval: Boolean; VAR CanRelease: Boolean; VAR CanReopen: Boolean);
    VAR
    ...
    PROCEDURE ViewApprovals(Rec: Record 7207286);
    VAR
    ...
    PROCEDURE SendApproval(Rec: Record 7207286);
    VAR
    ...
    PROCEDURE CancelApproval(Rec: Record 7207286);
    VAR
        WorkflowManagement: Codeunit 1501;
        WorkflowWebhookMgt: Codeunit 1543;
    BEGIN
    ...
    PROCEDURE WitHoldingApproval(Rec: Record 7207286);
    VAR
        QBApprovalManagement: Codeunit 7207354;
    ...
    BEGIN
        ...
        IF (RecRef.NUMBER = GetTableNumber) THEN BEGIN
            ...
            IF (TRUE) THEN BEGIN  //No hay varios tipos de documento en la misma tabla
                ...
                ApprovalEntryArgument.Type := 'Traspaso entre proyectos';
        ...
        IF (RecRef.NUMBER = GetTableNumber) THEN BEGIN
            ...
            IF (TRUE) THEN BEGIN  //No hay varios tipos de documento en la misma tabla
                ...
                END ELSE BEGIN
                    ...
                    IF (NOT Job.GET(Rec."QB Approval Job No")) THEN  //JAV 30/06/22: - QB 1.10.47 Se cambia el campo "Job No." por "QB Approval Job No"
                        Job.INIT;

                    Field1Label := 'Importe';
                    Field1Value := FORMAT(Amount);
                    Field2Label := ''; //El proyecto ya est� en la l�nea anterior
                    Field2Value := '';
                END;
            END;
    ...
    BEGIN
        ...
        //------------------------------------------------------------------------------------------------------------------------
        IF (RecRef.NUMBER = GetTableNumber) THEN BEGIN
            DocumentType := 'traspaso entre proyectos';
    ...
    PROCEDURE IsApprovalsWorkflowEnabled(VAR Rec: Record 7207286): Boolean;
    VAR
        WorkflowManagement: Codeunit 1501;
    BEGIN
    ...
    PROCEDURE CheckApprovalPossible(VAR Rec: Record 7207286): Boolean;
    VAR
    ...
    PROCEDURE PrePostApprovalCheck(VAR Rec: Record 7207286): Boolean;
    VAR
    ...
    BEGIN
        ...
        EXIT(QBApprovalManagement.GetApprovalsFixedText(pType, 'TRASPASO', 'un traspaso entre proyectos'));
    END;
    ...
    BEGIN
        ...
        RecRef.SETTABLE(Rec);
        EXIT(Rec."QB Approval Job No");  //JAV 30/06/22: - QB 1.10.47 Se cambia el campo "Job No." por "QB Approval Job No"
    END;
    ...
    BEGIN
        ...
        RecRef.SETTABLE(Rec);
        EXIT(Rec."QB Approval Budget item");  //JAV 30/06/22: - QB 1.10.47 Se usan los nuevos campos para proyecto, partida presupuestaria y departamento
    END;
    ...
    BEGIN
        ...
        RecRef.SETTABLE(Rec);
        EXIT(Rec."QB Approval Department");  //JAV 30/06/22: - QB 1.10.47 Se usan los nuevos campos para proyecto, partida presupuestaria y departamento
    END;
    ...
    BEGIN
        ...
        RecRef.SETTABLE(Rec);
        Amount := 0;      //No hay importes, se deben aprobar todos
        AmountLCY := 0;   //No hay divisas
    END;

    LOCAL PROCEDURE SetApprovalCircuit(VAR Rec: Record 7207286);
    VAR
        QBApprovalManagement: Codeunit 7207354;
        RecRef: RecordRef;
    BEGIN
    ...
    LOCAL PROCEDURE CheckAdditionalConditions(Rec: Record 7207286);
    BEGIN
    ...
    [EventSubscriber(ObjectType::Table, 7207286, OnAfterValidateEvent, "QB Approval Job No", true, true)]
    LOCAL PROCEDURE T7207286_OnAfterValidateEvent_JobNo(VAR Rec: Record 7207286; VAR xRec: Record 7207286; CurrFieldNo: Integer);
    BEGIN
        //JAV 07/11/22: - QB 1.12.14 Se arreglan funciones OnValidate para que usen los nuevos campos
        IF (TRUE) THEN  //No hay varios tipos de documento en la misma tabla
            SetApprovalCircuit(Rec);
    END;

    [EventSubscriber(ObjectType::Table, 7207286, OnAfterValidateEvent, "QB Approval Budget item", true, true)]
    LOCAL PROCEDURE T7207286_OnAfterValidateEvent_Budgetitem(VAR Rec: Record 7207286; VAR xRec: Record 7207286; CurrFieldNo: Integer);
    BEGIN
    ...
    /*{
          ...
          JAV 30/06/22: - QB 1.10.47 Si no tiene un circuito, lo buscamos antes de solicitar la aprobaci�n. Se lanza el evento OnAfterSendApproval antes de solicitar aprobaci�n
                                     Se cambia el campo "Job No." por "QB Approval Job No" y se usan los nuevos campos para proyecto, partida presupuestaria y departamento
