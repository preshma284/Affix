Codeunit 7206927 "Approval Payment Orders"
{
    ...
    //[IntegrationEvent]
    PROCEDURE OnAfterSendApproval(VAR Rec: Record 7000020);
    BEGIN
    END;

    //[IntegrationEvent]
    PROCEDURE OnAfterReleaseDocument(VAR Rec: Record 7000020);
    BEGIN
    END;

    //[IntegrationEvent]
    PROCEDURE OnAfterReopenDocument(VAR Rec: Record 7000020);
    BEGIN
    END;

    //[IntegrationEvent]
    PROCEDURE OnAfterPerformManualReopenDoc(VAR Rec: Record 7000020);
    BEGIN
    END;
    ...
    PROCEDURE Release(VAR Rec: Record 7000020);
    BEGIN
    ...
    PROCEDURE Reopen(VAR Rec: Record 7000020);
    BEGIN
    ...
    PROCEDURE PerformManualRelease(VAR Rec: Record 7000020);
    VAR
    ...
    PROCEDURE PerformManualReopen(VAR Rec: Record 7000020);
    VAR
    ...
    PROCEDURE SetApprovalsControls(Rec: Record 7000020; VAR ApprovalsActive: Boolean; VAR OpenApprovalEntriesExistForCurrUser: Boolean; VAR CanRequestApproval: Boolean; VAR CanCancelApproval: Boolean; VAR CanRelease: Boolean; VAR CanReopen: Boolean);
    VAR
    ...
    PROCEDURE ViewApprovals(Rec: Record 7000020);
    VAR
    ...
    PROCEDURE SendApproval(Rec: Record 7000020);
    VAR
    ...
    BEGIN
        ...
        RecRef.GETTABLE(Rec);
        QBApprovalManagement.CheckApprovalCircuit(GetDepCode(RecRef), GetCircuitCode(RecRef));
    ...
    PROCEDURE CancelApproval(Rec: Record 7000020);
    VAR
        WorkflowManagement: Codeunit 1501;
        WorkflowWebhookMgt: Codeunit 1543;
    BEGIN
    ...
    PROCEDURE WitHoldingApproval(Rec: Record 7000020);
    VAR
        QBApprovalManagement: Codeunit 7207354;
        ...
        IF (RecRef.NUMBER = GetTableNumber) THEN BEGIN
            ...
            IF (TRUE) THEN BEGIN  //No hay varios tipos de documento en la misma tabla
                ...
                END ELSE BEGIN
                    ...
                    Rec.CALCFIELDS("Bank Account Name");

                    Field1Label := 'Importe';
                    Field1Value := FORMAT(Amount);
                    Field2Label := 'Banco';
                    Field2Value := '(' + FORMAT(Rec."Bank Account No.") + ') ' + Rec."Bank Account Name";
                END;
            END;
    ...
    PROCEDURE IsApprovalsWorkflowEnabled(VAR Rec: Record 7000020): Boolean;
    VAR
        WorkflowManagement: Codeunit 1501;
    BEGIN
    ...
    PROCEDURE CheckApprovalPossible(VAR Rec: Record 7000020): Boolean;
    VAR
    ...
    PROCEDURE PrePostApprovalCheck(VAR Rec: Record 7000020): Boolean;
    VAR
    ...
    BEGIN
        ...
        EXIT(QBApprovalManagement.GetApprovalsFixedText(pType, 'ORDENPAGO', 'una Orden de Pago'));
    END;
    ...
    BEGIN
        ...
        EXIT('');  //No hay proyecto en las �rdenes de pago
    END;
    ...
    BEGIN
        ...
        EXIT('');  //No hay partida en las �rdenes de pago
    END;
    ...
    BEGIN
        ...
        RecRef.SETTABLE(Rec);
        EXIT(Rec."QB Department");
    END;

    LOCAL PROCEDURE GetDepCode(RecRef: RecordRef): Code[20];
    VAR
        Rec: Record 7000020;
    BEGIN
        //Retorna el c�digo del departamento relacionado con el documento
        RecRef.SETTABLE(Rec);

        //JAV 25/06/22: - QB 1.10.54 Para poder aprobar debe tener un departamento asociado
        IF (Rec."QB Department" = '') THEN
            ERROR('Debe indicar un departamento para la �rden de pago');
    ...
    BEGIN
        ...
        Amount := Rec.Amount;
        AmountLCY := Rec."Amount (LCY)";
    END;

    LOCAL PROCEDURE SetApprovalCircuit(VAR Rec: Record 7000020);
    VAR
        QBApprovalManagement: Codeunit 7207354;
        RecRef: RecordRef;
    BEGIN
    ...
    LOCAL PROCEDURE CheckAdditionalConditions(Rec: Record 7000020);
    BEGIN
    ...
    PROCEDURE VerifyApprovalStatus(Rec: Record 7000020);
    VAR
        Version: Record 7000020;
        Text001: TextConst ESP = 'La �rden de Pago debe estar aprobada para poderla Generar o Registrar.';
    BEGIN
        IF (IsApprovalsWorkflowActive) THEN
            IF (Rec."Approval Status" <> Rec."Approval Status"::Released) THEN
                ERROR(Text001);
    END;
    ...
    [EventSubscriber(ObjectType::Table, 7000020, OnAfterValidateEvent, "Job No.", true, true)]
    LOCAL PROCEDURE T7000020_OnAfterValidateEvent_JobNo(VAR Rec: Record 7000020; VAR xRec: Record 7000020; CurrFieldNo: Integer);
    BEGIN
        IF (TRUE) THEN  //No hay varios tipos de documento en la misma tabla
            SetApprovalCircuit(Rec);
    END;

    [EventSubscriber(ObjectType::Page, 7000050, OnNewRecordEvent, '', true, true)]
    LOCAL PROCEDURE PG7000050_OnNewRecordEvent(VAR Rec: Record 7000020; BelowxRec: Boolean; VAR xRec: Record 7000020);
    VAR
        QBDepartments: Record 7207002;
        QBOrganizationDepartment: Page 7207355;
    BEGIN
        //JAV 25/06/22: - QB 1.10.54 Al crear un nuevo registro de �rdenes de pago, buscar el departamento asociado
        IF (IsApprovalsWorkflowActive) THEN BEGIN
            QBDepartments.RESET;
            IF (QBDepartments.COUNT = 0) THEN BEGIN
                //No hay definido ningun departamento, por tanto no se podr� aprobar
                ERROR('No ha definido ning�n departamento, no podr� aprobar el documento, definalos antes de continuar.');
            END ELSE IF (QBDepartments.COUNT = 1) THEN BEGIN
                //Si solo hay un departamento definido, usaremos este
                QBDepartments.FINDFIRST;
                Rec."QB Department" := QBDepartments.Code;
                EXIT;
            END;

            QBDepartments.RESET;
            QBDepartments.SETRANGE("For Payments", TRUE);
            IF (QBDepartments.COUNT = 1) THEN BEGIN
                //Si solo hay un departamento para pagos definido, usaremos este
                QBDepartments.FINDFIRST;
                Rec."QB Department" := QBDepartments.Code;
                EXIT;
            END ELSE IF (QBDepartments.COUNT > 1) THEN BEGIN
                //Si hay varios departamentos de pagos, deben elegir cual
                CLEAR(QBOrganizationDepartment);
                QBOrganizationDepartment.SETTABLEVIEW(QBDepartments);
                QBOrganizationDepartment.LOOKUPMODE(TRUE);
                IF (QBOrganizationDepartment.RUNMODAL = ACTION::LookupOK) THEN BEGIN
                    QBOrganizationDepartment.GETRECORD(QBDepartments);
                    Rec."QB Department" := QBDepartments.Code;
                    EXIT;
                END;
            END;

            //Si no se ha seleccionado ning�n departamento, dar mensaje de que deben hacerlo manualmente
            MESSAGE('No ha seleccionado ning�n departamento, no podr� aprobar el documento.');
