Codeunit 7206915 "Approval Jobs"
{
    ...
    //[IntegrationEvent]
    PROCEDURE OnAfterSendApproval(VAR Rec: Record 167);
    BEGIN
    END;

    //[IntegrationEvent]
    PROCEDURE OnAfterReleaseDocument(VAR Rec: Record 167);
    BEGIN
    END;

    //[IntegrationEvent]
    PROCEDURE OnAfterReopenDocument(VAR Rec: Record 167);
    BEGIN
    END;

    //[IntegrationEvent]
    PROCEDURE OnAfterPerformManualReopenDoc(VAR Rec: Record 167);
    BEGIN
    END;
    ...
    PROCEDURE Release(VAR Rec: Record 167);
    BEGIN
    ...
    PROCEDURE Reopen(VAR Rec: Record 167);
    BEGIN
    ...
    PROCEDURE PerformManualRelease(VAR Rec: Record 167);
    VAR
    ...
    PROCEDURE PerformManualReopen(VAR Rec: Record 167);
    VAR
    ...
    PROCEDURE SetApprovalsControls(Rec: Record 167; VAR ApprovalsActive: Boolean; VAR OpenApprovalEntriesExistForCurrUser: Boolean; VAR CanRequestApproval: Boolean; VAR CanCancelApproval: Boolean; VAR CanRelease: Boolean; VAR CanReopen: Boolean);
    VAR
    ...
    PROCEDURE ViewApprovals(Rec: Record 167);
    VAR
    ...
    PROCEDURE SendApproval(Rec: Record 167);
    VAR
    ...
    PROCEDURE CancelApproval(Rec: Record 167);
    VAR
        WorkflowManagement: Codeunit 1501;
        WorkflowWebhookMgt: Codeunit 1543;
    BEGIN
    ...
    PROCEDURE WitHoldingApproval(Rec: Record 167);
    VAR
        QBApprovalManagement: Codeunit 7207354;
    ...
    BEGIN
        ...
        IF (RecRef.NUMBER = GetTableNumber) THEN BEGIN
            ...
            IF (TRUE) THEN BEGIN  //No hay varios tipos de documento en la misma tabla
                ...
                ApprovalEntryArgument.Type := 'Versi�n del Estudio';
                ApprovalEntryArgument."Payment Method Code" := '';
                ApprovalEntryArgument."Payment Terms Code" := '';
                ApprovalEntryArgument."QB_Piecework No." := '';  //JAV 03/11/22: - QB 1.12.12 A�ado la partida presupuestaria de aprobaci�n, aunque no tiene esta tabla
            END;
        END;
    END;
    ...
    BEGIN
        ...
        IF (RecRef.NUMBER = GetTableNumber) THEN BEGIN
            RecRef.SETTABLE(Rec);
            IF (TRUE) THEN BEGIN  //No hay varios tipos de documento en la misma tabla
                Rec.VALIDATE(Status, Rec."Approval Status"::"Pending Approval");
                Rec.MODIFY(TRUE);
                Variant := Rec;
                IsHandled := TRUE;
            END;
        ...
        IF (RecRef.NUMBER = GetTableNumber) THEN BEGIN
            ...
            IF (TRUE) THEN BEGIN  //No hay varios tipos de documento en la misma tabla
                ...
                END ELSE BEGIN
                    ...
                    Field2Label := 'Cliente';
                    Field2Value := '(' + FORMAT(Rec."Bill-to Customer No.") + ') ' + Rec."Bill-to Name";
                END;
            END;
    ...
    PROCEDURE IsApprovalsWorkflowEnabled(VAR Rec: Record 167): Boolean;
    VAR
        WorkflowManagement: Codeunit 1501;
    BEGIN
    ...
    PROCEDURE CheckApprovalPossible(VAR Rec: Record 167): Boolean;
    VAR
    ...
    PROCEDURE PrePostApprovalCheck(VAR Rec: Record 167): Boolean;
    VAR
    ...
    BEGIN
        ...
        EXIT(ApprovalEntry."QB Document Type"::Job);
    END;
    ...
    BEGIN
        ...
        EXIT(QBApprovalManagement.GetApprovalsFixedText(pType, 'PROYECTO', 'un Estudio/Proyecto'));
    END;
    ...
    BEGIN
        ...
        RecRef.SETTABLE(Rec);
        EXIT(FunctionQB.ReturnDepartmentOrganizationByDefDim(RecRef.NUMBER, Rec."No."));
    END;
    ...
    BEGIN
        ...
        RecRef.SETTABLE(Rec);
        Rec.CALCFIELDS("Amou Piecework Meas./Certifi.");
        Amount := Rec."Amou Piecework Meas./Certifi.";
        IF (Rec."Currency Code" = '') THEN
            AmountLCY := Amount
        ELSE
            AmountLCY := ROUND(Amount * Rec."Currency Factor (LCY)", 0.01);
    END;

    LOCAL PROCEDURE SetApprovalCircuit(VAR Rec: Record 167);
    VAR
        QBApprovalManagement: Codeunit 7207354;
        RecRef: RecordRef;
    BEGIN
    ...
    LOCAL PROCEDURE CheckAdditionalConditions(Rec: Record 167);
    BEGIN
    ...
    [EventSubscriber(ObjectType::Table, 167, OnAfterValidateEvent, "Presented Version", true, true)]
    PROCEDURE OnAfterValidateEvent_TJob_PresentedVersion(VAR Rec: Record 167; VAR xRec: Record 167; CurrFieldNo: Integer);
    VAR
        Version: Record 167;
        Text001: TextConst ESP = 'La versi�n debe estar aprobada para poderla presentar o aceptar.';
    BEGIN
        IF Version.GET(Rec."Presented Version") THEN
            VerifyApprovalStatus(Version);
    END;

    [EventSubscriber(ObjectType::Table, 167, OnAfterValidateEvent, "Accepted Version No.", true, true)]
    PROCEDURE OnAfterValidateEvent_TJob_AceptedVersion(VAR Rec: Record 167; VAR xRec: Record 167; CurrFieldNo: Integer);
    VAR
        Version: Record 167;
        Text001: TextConst ESP = 'La versi�n debe estar aprobada para poderla presentar o aceptar.';
    BEGIN
        IF Version.GET(Rec."Accepted Version No.") THEN
            VerifyApprovalStatus(Version);
    END;

    LOCAL PROCEDURE VerifyApprovalStatus(Rec: Record 167);
    VAR
        Version: Record 167;
        Text001: TextConst ESP = 'La versi�n debe estar aprobada para poderla presentar o aceptar.';
    BEGIN
    ...
    [EventSubscriber(ObjectType::Table, 167, OnAfterValidateEvent, "No.", true, true)]
    LOCAL PROCEDURE T167_OnAfterValidateEvent_JobNo(VAR Rec: Record 167; VAR xRec: Record 167; CurrFieldNo: Integer);
    BEGIN
