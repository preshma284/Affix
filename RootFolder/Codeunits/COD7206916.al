Codeunit 7206916 "Approval Comparative Quote"
{
    ...
    //[IntegrationEvent]
    PROCEDURE OnAfterSendApproval(VAR Rec: Record 7207412);
    BEGIN
    END;

    [IntegrationEvent(true, false)]
    PROCEDURE OnAfterReleaseDocument(VAR Rec: Record 7207412);
    BEGIN
    END;

    [IntegrationEvent(true, false)]
    PROCEDURE OnAfterReopenDocument(VAR Rec: Record 7207412);
    BEGIN
    END;

    //[IntegrationEvent]
    PROCEDURE OnAfterPerformManualReopenDoc(VAR Rec: Record 7207412);
    BEGIN
    END;
    ...
    PROCEDURE Release(VAR Rec: Record 7207412);
    BEGIN
    ...
    PROCEDURE Reopen(VAR Rec: Record 7207412);
    BEGIN
    ...
    PROCEDURE PerformManualRelease(VAR Rec: Record 7207412);
    VAR
    ...
    PROCEDURE PerformManualReopen(VAR Rec: Record 7207412);
    VAR
    ...
    PROCEDURE SetApprovalsControls(Rec: Record 7207412; VAR ApprovalsActive: Boolean; VAR OpenApprovalEntriesExistForCurrUser: Boolean; VAR CanRequestApproval: Boolean; VAR CanCancelApproval: Boolean; VAR CanRelease: Boolean; VAR CanReopen: Boolean);
    VAR
    ...
    PROCEDURE ViewApprovals(Rec: Record 7207412);
    VAR
    ...
    PROCEDURE SendApproval(Rec: Record 7207412);
    VAR
    ...
    PROCEDURE CancelApproval(Rec: Record 7207412);
    VAR
        WorkflowManagement: Codeunit 1501;
        WorkflowWebhookMgt: Codeunit 1543;
    BEGIN
    ...
    PROCEDURE WitHoldingApproval(Rec: Record 7207412);
    VAR
        QBApprovalManagement: Codeunit 7207354;
    ...
    BEGIN
        ...
        IF (RecRef.NUMBER = GetTableNumber) THEN BEGIN
            RecRef.SETTABLE(Rec);
            IF (TRUE) THEN BEGIN    // No hay varios tipos de documento en la tabla
    ...
    BEGIN
        ...
        IF (RecRef.NUMBER = GetTableNumber) THEN BEGIN
            ...
            IF (TRUE) THEN BEGIN    // No hay varios tipos de documento en la tabla
                ...
                Rec.CALCFIELDS("Payment Method Code", "Payment Terms Code");
                ApprovalEntryArgument.Type := 'Comparativo';
                ...
                ApprovalEntryArgument."QB_Piecework No." := GetBudgetItemCode(RecRef);  //JAV 14/12/22: - QB 1.12.27 Guardar la Partida Presupuestaria
            END;
        END;
    END;
        ...
        IF (RecRef.NUMBER = GetTableNumber) THEN BEGIN
            ...
            IF (TRUE) THEN BEGIN  //No hay varios tipos de documento en la misma tabla
                ...
                END ELSE BEGIN
                    ...
                    IF NOT Vendor.GET(Rec."Selected Vendor") THEN
                        Vendor.INIT;

                    Field1Label := 'Importe';
                    Field1Value := FORMAT(Amount);
                    Field2Label := 'Proveedor';
                    Field2Value := '(' + FORMAT(Rec."Selected Vendor") + ') ' + Vendor.Name;
                END;
            END;
    ...
    BEGIN
        ...
        //------------------------------------------------------------------------------------------------------------------------
        IF (RecRef.NUMBER = GetTableNumber) THEN BEGIN
            DocumentType := 'Comparativo de Compra';
    ...
    PROCEDURE IsApprovalsWorkflowEnabled(VAR Rec: Record 7207412): Boolean;
    VAR
        WorkflowManagement: Codeunit 1501;
    BEGIN
    ...
    PROCEDURE CheckApprovalPossible(VAR Rec: Record 7207412): Boolean;
    VAR
    ...
    PROCEDURE PrePostApprovalCheck(VAR Rec: Record 7207412): Boolean;
    VAR
    ...
    BEGIN
        ...
        EXIT(QBApprovalManagement.GetApprovalsFixedText(pType, 'COMPARATIVO', 'de un comparativo'));
    END;
    ...
    LOCAL PROCEDURE SetApprovalCircuit(VAR Rec: Record 7207412);
    VAR
        QBApprovalManagement: Codeunit 7207354;
        RecRef: RecordRef;
    BEGIN
    ...
    LOCAL PROCEDURE CheckAdditionalConditions(Rec: Record 7207412);
    VAR
        Text001: TextConst ESP = 'No puede lanzar la aprobaci�n sin un proveedor seleccionado';
    BEGIN
        //Verificar condiciones adicionales necesarias para verificar expresamente este tipo de documento

        //Debe tener un proveedor seleccionado
        IF (NOT Rec."Selection Made") THEN
            ERROR(Text001);

        //Verificar que el proveedor no est� bloqueado para comparativos
        CheckVendorComparativeBloqued(Rec);
    END;
    ...
    PROCEDURE CheckNeedApproval(Rec: Record 7207412; pMsg: Boolean): Boolean;
    VAR
        PaymentTerms: Record 3;
        Txt01: TextConst ESP = 'El uso de la forma de pago %1 requiere aprobaci�n';
        Txt02: TextConst ESP = 'El uso del m�todo de pago %1 requiere aprobaci�n';
        PaymentMethod: Record 289;
    BEGIN
        IF PaymentTerms.GET(Rec."Payment Terms Code") THEN BEGIN
            IF (PaymentTerms."QB Requires purchase approval") THEN BEGIN
                IF (pMsg) THEN
                    MESSAGE(Txt01, PaymentTerms);
                EXIT(TRUE);
            END;
        END;

        IF PaymentMethod.GET(Rec."Payment Terms Code") THEN BEGIN
            IF (PaymentMethod."Requires approval on purchases") THEN BEGIN
                IF (pMsg) THEN
                    MESSAGE(Txt02, PaymentMethod);
                EXIT(TRUE);
            END;
        END;
    ...
    PROCEDURE CheckVendorComparativeBloqued(Rec: Record 7207412);
    VAR
        VendorQualityData: Record 7207418;
        Text001: TextConst ENU = 'Error: Vendor %1 is blocked for comparatives. Motivation: %2.', ESP = 'El proveedor %1 esta bloqueado para comparativos en la actividad %2. Verifique su ficha de calidad.\Comentario: %3.';
    BEGIN
        //Verificar que el proveedor no est� bloqueado para comparativos
        VendorQualityData.RESET;
        VendorQualityData.SETRANGE("Vendor No.", Rec."Selected Vendor");
        VendorQualityData.SETFILTER("Activity Code", Rec."Activity Filter");
        IF (VendorQualityData.FINDSET(FALSE)) THEN
            REPEAT
                IF VendorQualityData."Comparative Blocked" THEN
                    ERROR(Text001, VendorQualityData."Vendor No.", VendorQualityData."Activity Code", VendorQualityData.Comment);
            UNTIL (VendorQualityData.NEXT = 0);
    END;

    [EventSubscriber(ObjectType::Codeunit, 7206916, OnAfterReleaseDocument, '', true, true)]
    LOCAL PROCEDURE Process_OnAfterReleaseDocument(VAR Rec: Record 7207412);
    BEGIN
        //JAV 05/06/22: - QB 1.10.47 Tras lanzar el documento se marca el estado
        Rec."Comparative Status" := Rec."Comparative Status"::Approved;
        Rec.MODIFY;
    END;

    [EventSubscriber(ObjectType::Codeunit, 7206916, OnAfterReopenDocument, '', true, true)]
    LOCAL PROCEDURE Process_OnAfterReopenDocument(VAR Rec: Record 7207412);
    BEGIN
        //JAV 05/06/22: - QB 1.10.47 Tras reabrir el documento se marca el estado
    ...
    [EventSubscriber(ObjectType::Table, 7207412, OnAfterValidateEvent, "Job No.", true, true)]
    LOCAL PROCEDURE T7207412_OnAfterValidateEvent_JobNo(VAR Rec: Record 7207412; VAR xRec: Record 7207412; CurrFieldNo: Integer);
    BEGIN
        IF (TRUE) THEN      // No hay varios tipos de documento en la tabla
            SetApprovalCircuit(Rec);
    END;

    [EventSubscriber(ObjectType::Table, 7207412, OnAfterValidateEvent, "QB Budget item", true, true)]
    LOCAL PROCEDURE T7207412_OnAfterValidateEvent_Budgetitem(VAR Rec: Record 7207412; VAR xRec: Record 7207412; CurrFieldNo: Integer);
    BEGIN
        IF (TRUE) THEN      // No hay varios tipos de documento en la tabla
            SetApprovalCircuit(Rec);
    END;
    ...
    /*{
          ...
          JAV 14/12/22: - QB 1.12.27 Guardar la Partida Presupuestaria
        }
    END.*/
}
