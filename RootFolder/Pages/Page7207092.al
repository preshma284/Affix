page 7207092 "RE Active Card"
{
    CaptionML = ENU = 'Real Estate Active Card', ESP = 'Ficha de Activo Inmobiliario';
    SourceTable = 167;
    SourceTableView = SORTING("No.")
                    ORDER(Ascending)
                    WHERE("Card Type" = FILTER("Promocion"));
    PageType = Card;
    ...
    actions
    {
        area(Navigation)
        {
            ...
            group("group11")
            {
                ...
                action("action8")
                {
                    CaptionML = ENU = 'Active', ESP = 'Activo';
                    Image = CompanyInformation;

                    trigger OnAction()
                    VAR
                        ProyectosInmobiliarios: Record 7238195;
                        Fichafasedeventaalquiler: Page 7219739;
                    BEGIN
                        ProyectosInmobiliarios.RESET;
                        IF (NOT ProyectosInmobiliarios.GET(ProyectosInmobiliarios.Tipo::"Fases de venta", Rec."No.")) THEN BEGIN
                            ProyectosInmobiliarios.INIT;
                            ProyectosInmobiliarios.Tipo := ProyectosInmobiliarios.Tipo::"Fases de venta";
                            ProyectosInmobiliarios."No." := Rec."No.";
                            ProyectosInmobiliarios.INSERT(TRUE);
                        END;

                        CLEAR(Fichafasedeventaalquiler);
                        Fichafasedeventaalquiler.SETRECORD(ProyectosInmobiliarios);
                        Fichafasedeventaalquiler.RUNMODAL;
                    END;
    ...
    BEGIN
        ...
        JobCurrencyExchangeFunction.SetPageCurrencies(useCurrencies, 2);
    ...
    BEGIN
        ...
        rec."Card Type" := rec."Card Type"::Promocion; // QB4973
    ...
    /*{
      ...
      JAV 07/04/22: - QB 1.10.32 RE 1.00.09 Si el proyecto puede activar gastos
      ...
      PSM 19/10/22: - Se muestra el campo "Bill-to Customer No." en el Group "Posting"
    }*///end
}
