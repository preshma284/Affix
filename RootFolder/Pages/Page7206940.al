page 7206940 "QB Efectos anticipados Liq"
{
  ApplicationArea=All;

    CaptionML = ESP = 'Efectos anticipados liquidados';
    InsertAllowed = false;
    DeleteAllowed = false;
    ModifyAllowed = false;
    SourceTable = 7206923;
    SourceTableView = SORTING("Relacion No.", "Document Type", "Document No.", "Bill No.")
                    WHERE("Registrada" = CONST(true), "Tipo Linea" = CONST("Anticipado"), "Anticipo Estado" = FILTER(<> ' '));
    ...
    layout
    {
        area(content)
        {
            repeater("table")
            {
                ...
                field("Anticipo Estado"; rec."Anticipo Estado")
                {

                }
                field("Anticipo liquidado por"; rec."Anticipo liquidado por")
                {

                }
                field("Anticipo liquidado en fecha"; rec."Anticipo liquidado en fecha")
                {
                ...
                field("No. Mov. del Anticipado"; rec."No. Mov. del Anticipado")
                {
